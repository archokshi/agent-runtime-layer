...
else { Err "Backend not healthy (status: $health) -- run: docker compose up -d" }

$dashV3 = docker inspect --format="{{.State.Status}}" agent-runtime-layer-customer-dashboard-v3-1 2>$null
if ($dashV3 -eq "running") { Ok "customer-dashboard-v3 running" }
else { Err "customer-dashboard-v3 not running (status: $dashV3)" }

# -- 2. API endpoints -------------------------------------------------------
Step "2. API endpoints (port 8000)"
...
$dashRoutes = @("/overview", "/runs", "/settings", "/bottlenecks", "/context", "/cost", "/recommendations")
...
foreach ($route in $dashRoutes) {
    try {
        ...
        if ($res.StatusCode -in @(200, 307)) { Ok "localhost:4001$route" }
...
try {
    ...
    try {
        ...
        if ($_.Exception.Response.StatusCode.value__ -eq 403) { $blocked = $true }
    }
    if ($blocked) { Ok "Plan gate blocks optimizer on free plan (403)" }
...
# -- 5. Docker Hub images ---------------------------------------------------
Step "5. Docker Hub images (local cache)"
...
foreach ($img in $images) {
    ...
    else { Err "Image not cached locally: $img -- run: docker pull $img" }
}

# -- 6. Vercel install page -------------------------------------------------
